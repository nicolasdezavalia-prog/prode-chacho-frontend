/**
 * MundialFixtureImpacto — "¿Qué se juega?" landing (v3 con EN VIVO).
 */
...
const RONDA_LABEL = {
  '16vos': '16avos', '8vos': '8vos', '4tos': 'Cuartos',
  'semis': 'Semis', 'tercer_puesto': '3er puesto', 'final': 'Final',
}
const RONDA_CHIP = {
  '16vos': '16AVOS', '8vos': 'OCTAVOS', '4tos': 'CUARTOS',
  'semis': 'SEMIS', 'tercer_puesto': '3ER P.', 'final': 'FINAL',
}
...
export default function MundialFixtureImpacto() {
  ...
  const partidoDestacado = useMemo(() => {
    ...
    let best = data.por_venir[0], bestMax = 0
    ...
    <div style={{ maxWidth: 720, margin: '24px auto', padding: '0 16px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700 }}>Fixture — {torneo?.nombre}</h1>
          <div style={{ fontSize: 12, color: 'var(--color-muted)', marginTop: 4 }}>
            ¿Qué se juega hoy? Los próximos partidos y qué podés sumar.
          </div>
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          <Link to={`/mundial/${torneoId}/mi-mundial`} className="btn btn-secondary btn-sm">Mi Mundial</Link>
          <Link to={`/mundial/${torneoId}/ranking`} className="btn btn-secondary btn-sm">Ranking</Link>
        </div>
      </div>
      ...
      {enGrupos && (
        <div style={emptyStyle}>⚽ El torneo está en fase de grupos. Esta vista se activa cuando arrancan los 16avos.</div>
      )}
      {sinRonda && !enGrupos && (
        <div style={emptyStyle}>🎉 No hay más partidos pendientes. ¡Terminó el Mundial!</div>
      )}
      ...
      {(data.jugados_hoy || []).length > 0 && <JugadosHoy items={data.jugados_hoy} />}

      <div style={{
        marginTop: 20, padding: '12px 14px', background: 'white',
...
function HeroTuJornada({ data }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 12, padding: 16, marginBottom: 22, boxShadow: '0 1px 3px rgba(0,0,0,0.04)',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
        <div style={{
          width: 40, height: 40, borderRadius: 20, background: '#fbbf24', color: '#78350f',
          display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, fontWeight: 800,
        }}>{(u?.nombre || 'Y').charAt(0).toUpperCase()}</div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 10, color: 'var(--color-muted)', letterSpacing: 1.5, fontWeight: 700 }}>TU JORNADA</div>
...
function HeroMetric({ label, valor, sub, color, tint }) {
  return (
    <div style={{
      ...
      textAlign: 'center', border: color ? `1px solid ${color}55` : 'none',
    }}>
      <div style={{ fontSize: 9, color: 'var(--color-muted)', letterSpacing: 1, fontWeight: 700 }}>{label}</div>
      <div style={{ fontSize: 22, fontWeight: 800, color: color || 'var(--color-text)', lineHeight: 1, marginTop: 4 }}>{valor}</div>
      {sub && <div style={{ fontSize: 10, color: 'var(--color-muted)', marginTop: 4 }}>{sub}</div>}
    </div>
  )
}
...
function SeparadorFecha({ texto }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '10px 0 12px' }}>
      ...
      <div style={{ fontSize: 10, color: 'var(--color-muted)', letterSpacing: 2, fontWeight: 700 }}>{texto}</div>
      <div style={{ height: 1, background: 'var(--color-border)', flex: 1 }} />
    </div>
  )
}
...
function PartidoCard({ partido, data, destacado, defaultAbierto }) {
  ...
  const hayGolesEnVivo = enVivo && (partido.goles_local != null || partido.goles_visitante != null)

  return (
    <div style={{
      background: 'white',
      border: enVivo ? '1px solid #dc2626' : (destacado ? '1px solid rgba(180,83,9,0.35)' : '1px solid var(--color-border)'),
      ...
      <div
        ...
        style={{
          ...
          background: enVivo ? 'rgba(220,38,38,0.05)' : (destacado ? 'rgba(251,191,36,0.06)' : '#f8fafc'),
      ...
      >
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {enVivo && (
              <span style={{
                ...
                animation: 'mfi-pulse 1.5s ease-in-out infinite',
              }}>🔴 EN VIVO</span>
            )}
          </span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'center' }}>
            ...
            {hayGolesEnVivo ? (
              <span style={{
                fontSize: 18, fontWeight: 800, padding: '2px 10px',
      ...
      {abierto && (
        <div style={{ padding: '14px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <EscenarioCard equipo={partido.equipo_local} equipoElim={partido.equipo_visitante}
              escenario={gL} userIdActual={data.user_actual?.user_id} ronda={partido.ronda} />
            <EscenarioCard equipo={partido.equipo_visitante} equipoElim={partido.equipo_local}
              escenario={gV} userIdActual={data.user_actual?.user_id} ronda={partido.ronda} />
          </div>
          {partido.preguntas_en_juego?.length > 0 && (
            <div style={{
              marginTop: 10, padding: '8px 10px', background: '#fef3c7', borderRadius: 6,
  ...
  return (
    <div style={{ background: bg, border: `1px solid ${border}`, borderRadius: 10, padding: '12px 10px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <div style={{ fontSize: 10, color, letterSpacing: 1, fontWeight: 700 }}>SI GANA {equipo}</div>
        <div style={{ fontSize: 20, fontWeight: 800, color }}>{escenario.delta_yo > 0 ? `+${escenario.delta_yo}` : '+0'}</div>
      </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 8 }}>
          {beneficiados.map(b => {
            ...
            return (
              ...
              }}>{esVos ? '✓ VOS' : `${b.nombre} +${b.delta_pts}`}</span>
            )
          })}
        ...
        <div style={{ marginTop: 8 }}>
          {items.map(p => (
            <div key={p.partido_id} style={{
              ...
              <span style={{ fontSize: 10, color: '#b45309', fontWeight: 700, letterSpacing: 1, width: 60 }}>{RONDA_CHIP[p.ronda]}</span>
              <Bandera codigo={p.equipo_local} width={22} height={14} />
              <span style={{ fontSize: 13, fontWeight: 600, minWidth: 40 }}>{p.equipo_local}</span>
              <span style={{ fontSize: 14, fontWeight: 800, fontVariantNumeric: 'tabular-nums' }}>{p.goles_local ?? '-'} - {p.goles_visitante ?? '-'}</span>
              ...
              {p.pts_sumaste_yo != null && (
                ...
                }}>{p.pts_sumaste_yo > 0 ? '+' + p.pts_sumaste_yo + ' pts' : '+0'}</span>
              )}
            </div>
          ))}
        </div>
...
const emptyStyle = {
  ...
  borderRadius: 8, fontSize: 14, lineHeight: 1.5, marginBottom: 12,
}
